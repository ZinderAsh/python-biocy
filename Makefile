...
clean-c: ## clean files generated by C and C++
	rm -rf kmer_finder tests/test_kivs $(CBUILDDIR)
...
test-c: $(CTESTDIR)/test_kivs ## run C tests
	$(CTESTDIR)/test_kivs

coverage: ## check code coverage quickly with the default Python
	coverage run --source kivs -m pytest
	coverage report -m
	coverage html
...
$(CTESTDIR)/test_kivs: $(CSRCDIR)/test_kivs.cpp $(COBJECTS) $(CHEADERS)
	mkdir -p $(CTESTDIR)
	$(CXX) $(CFLAGS) -o $@ $< $(COBJECTS) $(CHEADERS) -I.

kivs: $(CSRCDIR)/kivs.cpp $(COBJECTS) $(CHEADERS)
	mkdir -p $(CPROGRAMDIR)
